...
import { ObjectView, type SeqOpts } from './components/ObjectView'
...
export default function App() {
  ...
  const renameObject = useCallback(
    ...
    [tabs, dialogs, markTxn, loadObjectDef, loadExplorer],
  )
          ...
          <div className="min-h-0 flex-1 overflow-auto p-3">
            ...
            {cur?.kind === 'object' && (
              <ObjectView
                ...
                onSaveSequence={(o) => saveSequence(cur.id, o)}
                onSaveFunction={(sql) => saveFunction(cur.id, sql)}
                onEnumAdd={(v) => enumAdd(cur.id, v)}
                onRenameRequest={() => renameObject(cur.id)}
                onDropRequest={() => dropObject(cur.id)}
              />
            )}
