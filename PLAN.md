...
Frontend (`web/`):
- [x] Explorer: Databases node + per-schema groups (Tables/Views/MatViews/Foreign/Functions/Sequences/Types) + global Extensions/Roles nodes. Lazy per-schema loading, filter, counts. Right-click context menus (Radix `ui/context-menu`) at database/schema/table tiers: scoped New Query (`SET search_path` + `SELECT … LIMIT 100`), New Schema/Table dialogs, Open Data, Export CSV/INSERTs (≤1000 rows), Copy name, Refresh. Table/view DDL lives in the table workspace DDL sub-tab. No new backend endpoints.
