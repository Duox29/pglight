...
- [x] Query Manager history — additive SQLite migration for connection/database, statement type, success/error, and pinned state; capped SQL/error text, server-side parameterized filters and paging; UI records failed and multi-statement scripts and supports search, pin, copy, snippet, reopen, and delete.
...
POST /api/table-changes    {session_id,schema,table,updates[],inserts[],deletes[]} — atomic mixed table edits, optimistic original-value check
