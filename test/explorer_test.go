...
func TestDDLUsesCatalogTypesAndDefaults(t *testing.T) {
	h, sid := newHandler(t)
	tbl := tempTable(t)
	execSQL(t, h, sid, fmt.Sprintf(`CREATE TABLE %s (id bigint GENERATED BY DEFAULT AS IDENTITY, name varchar(50) COLLATE "C", amount numeric(18,6), created_at timestamptz(3), labels integer[] DEFAULT ARRAY[]::integer[], computed integer GENERATED ALWAYS AS (id::integer) STORED)`, tbl))
	defer execSQL(t, h, sid, fmt.Sprintf(`DROP TABLE %s`, tbl))

	code, body := callGET(t, h.DDL, withSID(sid, "/api/ddl?schema=public&table="+tbl))
	requireStatus(t, body, code, 200)
	ddl := fmt.Sprint(decodeObj(t, body)["ddl"])
	for _, want := range []string{
		`name character varying(50)`,
		`amount numeric(18,6)`,
		`created_at timestamp(3) with time zone`,
		`labels integer[] DEFAULT ARRAY[]::integer[]`,
		`id bigint GENERATED BY DEFAULT AS IDENTITY`,
		`computed integer GENERATED ALWAYS AS ((id)::integer) STORED`,
	} {
		if !strings.Contains(ddl, want) {
			t.Fatalf("catalog DDL missing %q: %s", want, body)
		}
	}
}
