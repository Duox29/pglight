...
func (h *Handler) DDL(w http.ResponseWriter, r *http.Request) {
	...
	err := q.QueryRow(ctx, `SELECT 'CREATE TABLE ' || quote_ident($1) || '.' || quote_ident($2) || E' (\n' ||
		...
		CASE WHEN a.attcollation <> 0 AND a.attcollation <> ty.typcollation
			THEN ' COLLATE ' || quote_ident(cn.nspname) || '.' || quote_ident(cc.collname) ELSE '' END ||
		...
		LEFT JOIN pg_collation cc ON cc.oid = a.attcollation
		LEFT JOIN pg_namespace cn ON cn.oid = cc.collnamespace
		WHERE n.nspname=$1 AND c.relname=$2 AND a.attnum > 0 AND NOT a.attisdropped`, schema, table).Scan(&ddl)
	if err != nil {
		writeJSON(w, 500, map[string]string{"error": err.Error()})
		return
	}
